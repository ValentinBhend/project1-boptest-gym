...
- Add a ``warmup_interval`` argument to ``BoptestGymEnv``, sent when initializing on each episode reset, which coarsens the warmup simulation. Default is ``30``, the previous behaviour. A BOPTEST that does not support it ignores it. This is for [#XXX](https://github.com/ibpsa/project1-boptest-gym/issues/XXX).
